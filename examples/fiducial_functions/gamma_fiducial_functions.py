...
def l2_jac_func(theta, data_matrix):
    ...
    jacobian_matrix = np.array([gamma_cdf_deriv_wrt_alpha(theta[0], data_matrix[0] * theta[1]) / \
                                (theta[1] * (
                                             (1 / scipy.special.gammaln(theta[0])) *
                                             theta[1] ** theta[0] *
                                             data_matrix[0] ** (theta[0] - 1) *
                                             np.exp(-data_matrix[0] * theta[1]))),
                                -data_matrix[0] * (1 / theta[1])]).reshape(1, 2)

    for index in range(1, data_matrix.shape[0]):
        jac_mat_temp_row = np.array([gamma_cdf_deriv_wrt_alpha(theta[0], data_matrix[index] * theta[1]) /
                                     (theta[1] * (
                                             (1 / scipy.special.gammaln(theta[0])) *
                                             theta[1] ** theta[0] *
                                             data_matrix[index] ** (theta[0] - 1) *
                                             np.exp(-data_matrix[index] * theta[1]))),
                                     -data_matrix[index] * (1 / theta[1])])
        jacobian_matrix = np.concatenate(
            (jacobian_matrix, jac_mat_temp_row.reshape(1, 2)), axis=0)
...
def log_likelihood(theta, data):
    ...
    for index in range(data.shape[0]):
        log_sum += - scipy.special.gammaln(theta[0]) + theta[0] * np.log(theta[1]) + \
                   (theta[0] - 1) * np.log(data[index]) - data[index] * theta[1]
    return log_sum
